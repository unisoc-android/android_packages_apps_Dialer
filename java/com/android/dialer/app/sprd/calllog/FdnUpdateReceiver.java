...
public class FdnUpdateReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (!context.getResources().getBoolean(R.bool.is_support_fdn)) {
            return;
        }
        ...
        if (ACTION_FDN_STATUS_CHANGED.equals(action)) {
            ...
            if (TelephonyManagerEx.from(context).getIccFdnEnabled(subId)) {
                DialerTelcelHelper.getInstance().queryFdnList(subId, context);
            } else {
                DialerTelcelHelper.getInstance().refreshFdnListCache(subId, context);
            }
            ...
            if (INTENT_VALUE_ICC_LOADED.equals(state)) {
                if (TelephonyManagerEx.from(context).getIccFdnEnabled(subId)) {
                    ...
                    DialerTelcelHelper.getInstance().refreshFdnListCache(
                            subId, context);
                }
